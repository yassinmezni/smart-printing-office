...
QSqlQuery qry;
int i=0;
qry.exec("SELECT QT_COMMANDES FROM COMMANDES");
while (qry.next())
{
...
QLinearGradient gradient(0, 0, 0, 400);
                  ...
                  amande->setStackingGap(1);
                   //couleurs
                  amande->setName("Repartition des quatités selon le prix ");
                  ...
                  // ajout des données  (statistiques du prix)//

                  QVector<double> PlaceData;
                  QSqlQuery q1("SELECT PRIX FROM COMMANDES");
                  while (q1.next()) {
